...
jQuery(window).on("wheel", function(e) {
    ...
    if (a == 0) {
        jQuery("#azCloseBtn").css('display','none');
          let url =   window.location.href.split('/');
    if (url[3]==='casestudies' ){

            jQuery('.navbar-toggler').hide();
    }
        jQuery("#backBtn").removeAttr('style');
    }
    ...
    if (url[3]==='casestudies' ){
        ...
        jQuery("#azCloseBtn").removeAttr('style');
...
function nextBtnFunction(input){
    jQuery(input).addClass('activeBtn');
    jQuery(input).removeClass('activeBtn');
...
// jQuery(document).on("click", 'button#nextBtnText', function() {
//     alert('213213')
...
jQuery(document).on("click", 'button#azCloseBtn', function() {


    jQuery("#TextVal").val('0');
    jQuery("#TextStop").val('1');

    //jQuery("#prevBtnText").css({opacity:0.4,cursor:'not-allowed'});

    jQuery(this).css('display','none');
    jQuery('.navbar-toggler').show()
    jQuery(".Screens").removeClass('active_screen');
    jQuery(".homeScreen").removeClass('prev-screen');
    jQuery(".homeScreen").addClass('active_screen');

    jQuery(".homeScreen").next('.Screens').addClass('next-screen');
    
    
});

// on click scrollUp
function prevBtnFunction(input){
    jQuery(input).addClass('activeBtn');
    jQuery(input).removeClass('activeBtn');
    var a = parseInt(jQuery("#TextVal").val());
    var b = parseInt(jQuery("#TextStop").val());
...
AZ cPrev 
*/
function leftScroll(input){
    // jQuery('#azCustomPrevBtnUpdated').unbind('click').click(function(e){
    //     e.preventDefault();
...
jQuery(document).ready(function() {
    ...
    jQuery('.overlay_text').click(function () {
        jQuery('.navbar-toggler').hide();
    });

    jQuery(document).unbind('click').on('click','#azCustomNextBtn',function(e){
        e.preventDefault();
        //alert(e);
        jQuery(this).addClass('activeBtn');
        jQuery(this).removeClass('activeBtn');
